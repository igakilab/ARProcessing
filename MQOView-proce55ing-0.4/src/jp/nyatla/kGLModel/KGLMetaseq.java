...
//-------
//	readVertexL  = readVertex
// 左手座標用(Processingとか.... Zを(-1)かけてる以外同じ
//	written by reco 2009/2/10
//-------
	private KGLPoint[] readVertexL(int num, multiInput br, float scale) throws Exception
	{
	    KGLPoint[] ret = null;
	    String line = null;
	    String[] s;
...
//-------
//	readBVertexL  = readBVertex
// 左手座標用(Processingとか.... Zを(-1)かけてる以外同じ
//	written by reco 2009/2/10
//-------
private KGLPoint[] readBvertexL(multiInput br, float scale)
	throws Exception {
	    KGLPoint[] ret = null;
	    String line = null;
	    String[] s;
...
//-------
//	readFaceL  = readBVertex
// 左手座標用(Processingとか....
//	頂点とUVデータの格納順序の変更。
// 反時計回りで格納。
// 0->1->2 を 0->2->1に。
// 参考のため、元ソース(for openGL)も残してある。
//	written by reco 2009/2/10
//-------
	private Face[] readFaceL(multiInput br) throws Exception
	{
	    ArrayList<Face> qf;
	    String line = null;
	    String[] s;
		...
		while ((line = br.readLine()) != null) {
			...
			wface[0].V[0] = Integer.parseInt(s[0]);
// ---- Processing対応。逆回りに格納
			wface[0].V[1] = Integer.parseInt(s[2]);
			wface[0].V[2] = Integer.parseInt(s[1]);
// ---- OpenGLの場合
//			wface[0].V[1] = Integer.parseInt(s[1]);
//			wface[0].V[2] = Integer.parseInt(s[2]);
			wface[0].M = Mn;
...
// ---- Processing対応。逆回りに格納
				wface[0].UV[0] = Float.parseFloat(s[0]);
				wface[0].UV[1] = Float.parseFloat(s[1]);
				wface[0].UV[2] = Float.parseFloat(s[4]);
				wface[0].UV[3] = Float.parseFloat(s[5]);
				wface[0].UV[4] = Float.parseFloat(s[2]);
				wface[0].UV[5] = Float.parseFloat(s[3]);
/*
			    for (int i = 0; i < s.length; i++) {
				wface[0].UV[i] = Float.parseFloat(s[i]);
			    }
...
//			wface[0].V[2] = Integer.parseInt(s[2]);
			...
			// ---- Processing対応。逆回りに格納
			wface[1].V[0] = Integer.parseInt(s[0]);
			wface[1].V[1] = Integer.parseInt(s[3]);
			wface[1].V[2] = Integer.parseInt(s[2]);
			// ---- OpenGL対応。逆回りに格納
//			wface[1].V[0] = Integer.parseInt(s[0]);
//			wface[1].V[1] = Integer.parseInt(s[2]);
//			wface[1].V[2] = Integer.parseInt(s[3]);
			...
			if (p != -1) {
				...
				wface[0].UV[4] = Float.parseFloat(s[2]);
				wface[0].UV[5] = Float.parseFloat(s[3]);
				wface[1].UV[0] = Float.parseFloat(s[0]);
				wface[1].UV[1] = Float.parseFloat(s[1]);
				wface[1].UV[2] = Float.parseFloat(s[6]);
				wface[1].UV[3] = Float.parseFloat(s[7]);
				wface[1].UV[4] = Float.parseFloat(s[4]);
				wface[1].UV[5] = Float.parseFloat(s[5]);
// OpenGL
/*
			    for (int i = 0; i < 2; i++) {
				uv_p = 0;
...
//	setL(...)
// 左手座標の読み込み
// 基本的には、上記のset関数と同じだが、読み込み部の呼び出し関数が異なる。
//	written by reco 2009/2/10
//---------
	
	private void setL(String in_name, multiInput br, float scale)throws Exception
	{
	    String line[];
	    name = getDoubleQuoatString(in_name);
	    while ((line = Chank(br, true)) != null) {
		...
		if (line[line.length - 1].equals("{")) { // 内部チャンク
		    if (line[0].equals("vertex")) {
			vertex = readVertexL(Integer.parseInt(line[1]), br,
				scale);
			continue;
		    }
		    if (line[0].equals("BVertex")) {
			vertex = readBvertexL(br, scale);
			continue;
		    }
		    if (line[0].equals("face")) {
			face = readFaceL(br);
	...
	// Modified by reco. some enviroment cannot use glGenBuffers. But isFunciton... return true..mmm
	//	boolean ret2 = gl.isFunctionAvailable("glGenBuffers"); 
//	if(ret2)
	{
		gl.glGenBuffersARB(ret.mat.length, ret.VBO_ids, 0);
	}
//	else
//	{
//		gl.glGenBuffersARB(ret.mat.length, ret.VBO_ids, 0);	
...
//    }
    ...
    protected KGLMetaseq(GL in_gl, KGLTextures in_texPool,ContentProvider i_provider,float scale, boolean isUseVBO, boolean ax)
    {
...
//	左手座標系なので。
//	modified by reco 2009/2/10
//
			if(ax == true)
			{
				object.setL(chankName[1], br, scale);
			}
			else
			{
				object.set(chankName[1], br, scale);
